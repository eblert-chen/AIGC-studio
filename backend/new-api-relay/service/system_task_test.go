...
func TestSystemTaskRunnerCoordinatorCancelsJoinsAndRestarts(t *testing.T) {
	truncate(t)
	started := make(chan struct{})
	canceled := make(chan struct{})
	handler := &stubScheduledHandler{
		taskType: "test_coordinator_cancel",
		enabled:  false,
		interval: time.Minute,
	...
	coordinator := &systemTaskRunnerCoordinator{}
	coordinator.start()
	select {
	case <-started:
	case <-time.After(2 * time.Second):
	...
	go func() { results <- coordinator.stop(stopContext) }()
	go func() { results <- coordinator.stop(stopContext) }()
	for range 2 {
	...
	case <-time.After(time.Second):
		t.Fatal("system task handler did not observe root cancellation")
	}

	coordinator.mu.Lock()
	require.False(t, coordinator.running)
	coordinator.mu.Unlock()

	// A stopped coordinator is restartable. With no pending work, it still
	// starts and joins its scheduler loop cleanly.
	truncate(t)
	coordinator.start()
	restartContext, cancelRestart := context.WithTimeout(context.Background(), time.Second)
	defer cancelRestart()
	require.NoError(t, coordinator.stop(restartContext))
}
