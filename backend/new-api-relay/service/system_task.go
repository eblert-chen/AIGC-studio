...
var (
	...
	systemTaskRunner = &systemTaskRunnerCoordinator{}
)

type systemTaskRunnerCoordinator struct {
	mu      sync.Mutex
	running bool
...
func StartSystemTaskRunner() {
	...
	systemTaskRunner.start()
}

func (coordinator *systemTaskRunnerCoordinator) start() {
	coordinator.mu.Lock()
	...
	// The runner itself keeps the count non-zero while it can add handler
	// goroutines, so Wait never races with a new Add. Cancellation stops claim
	// dispatch first; completion closes only after every in-flight handler and
	// lease heartbeat has observed the same root context and returned.
	var workers sync.WaitGroup
	workers.Add(1)
	go func() {
		defer workers.Done()
		runSystemTaskRunner(ctx, runnerID, func(run func()) {
			workers.Add(1)
			go func() {
				defer workers.Done()
				run()
...
func runSystemTaskRunner(ctx context.Context, runnerID string, launch func(func())) {
	...
	runPass := func() {
		...
		if now.Sub(lastStaleLockCleanup) >= systemTaskStaleLockInterval {
			...
			if err := model.ExpireStaleSystemTaskLocks(common.GetTimestamp()); err != nil && ctx.Err() == nil {
		...
		if now.Sub(lastScheduler) >= systemTaskSchedulerInterval {
			lastScheduler = now
			runSystemTaskSchedulerWithContext(ctx)
		}
		runSystemTaskClaimPassWithContext(ctx, runnerID, launch)
	}
...
// StopSystemTaskRunner cancels the scheduler, every claimed handler and its
// lease heartbeat, then waits for all of them to finish. Lifecycle-loss callers
// pass their shared hard-deadline context so external polling cannot outlive
// the database pool.
func StopSystemTaskRunner(ctx context.Context) error {
	return systemTaskRunner.stop(ctx)
}

func (coordinator *systemTaskRunnerCoordinator) stop(ctx context.Context) error {
	if ctx == nil {
		return errors.New("system task runner stop context is required")
	}
	coordinator.mu.Lock()
	if !coordinator.running {
		coordinator.mu.Unlock()
...
func runSystemTaskClaimPass(runnerID string) {
	runSystemTaskClaimPassWithContext(context.Background(), runnerID, func(run func()) {
		gopool.Go(run)
	})
}

func runSystemTaskClaimPassWithContext(ctx context.Context, runnerID string, launch func(func())) {
	if ctx == nil || ctx.Err() != nil || launch == nil {
		return
	}
	...
	if err != nil {
		if ctx.Err() == nil {
			logger.LogWarn(ctx, fmt.Sprintf("system task runner query failed: %v", err))
		}
		return
	}
	for _, handler := range handlers {
		...
		if err != nil {
			if ctx.Err() == nil {
				logger.LogWarn(ctx, fmt.Sprintf("system task claim failed: %v", err))
			}
			continue
		}
		if !claimed {
...
func runSystemTaskScheduler() {
	runSystemTaskSchedulerWithContext(context.Background())
}

func runSystemTaskSchedulerWithContext(ctx context.Context) {
	...
	if err != nil {
		if ctx.Err() == nil {
			logger.LogWarn(ctx, fmt.Sprintf("system task scheduler query failed: %v", err))
		}
		return
	}
	for _, scheduled := range scheduledHandlers {
		...
		if _, err := model.CreateSystemTask(scheduled.Type(), scheduled.NewPayload(), nil); err != nil {
			...
			if activeErr != nil {
				logger.LogWarn(ctx, fmt.Sprintf("system task scheduler active lookup failed: type=%s err=%v", scheduled.Type(), activeErr))
			}
			logger.LogWarn(ctx, fmt.Sprintf("system task scheduler create failed: type=%s err=%v", scheduled.Type(), err))
			continue
		}
	}
}
