...
func TestMain(m *testing.M) {
	_ = os.Setenv("RELAY_PROVIDER_CREDENTIAL_KEYRING_JSON", `{"schema_version":1,"active_key_id":"test-v1","keys":{"test-v1":"MDEyMzQ1Njc4OWFiY2RlZjAxMjM0NTY3ODlhYmNkZWY="}}`)
	...
	if !db.Migrator().HasTable(&model.ProviderChannelCredentialSetVersion{}) {
		panic("provider channel credential set version test table is missing")
	}
...
func truncate(t *testing.T) {
	...
	t.Cleanup(func() {
		...
		model.DB.Exec("DELETE FROM provider_credential_versions")
		model.DB.Exec("DELETE FROM users")
		model.DB.Exec("DELETE FROM setups")
		...
		model.DB.Exec("DELETE FROM provider_channel_credential_set_versions")
		model.DB.Exec("DELETE FROM top_ups")
		model.DB.Exec("DELETE FROM user_subscriptions")
		model.DB.Exec("DELETE FROM system_task_locks")
		model.DB.Exec("DELETE FROM system_tasks")
		model.DB.Exec("DELETE FROM platform_generation_route_admissions")
		model.DB.Exec("DELETE FROM platform_generation_provider_routes")
		model.DB.Exec("DELETE FROM platform_generation_provider_account_states")
		model.DB.Exec("DELETE FROM platform_generation_outboxes")
		model.DB.Exec("DELETE FROM platform_artifact_upload_intents")
		model.DB.Exec("DELETE FROM platform_generation_jobs")
	})
}
...
func TestPlatformExternalTaskBillingNeverMutatesNativeLedger(t *testing.T) {
	truncate(t)
	const (
		userID         = 9801
		tokenID        = 9802
	...
	seedToken(t, tokenID, userID, "sk-platform-external", 8000)
	seedSubscription(t, subscriptionID, userID, 9000, 100)
	task := &model.Task{
		TaskID:    "task_pg_external_billing",
		UserId:    userID,
		ChannelId: 12,
		Quota:     0,
	...
	require.NoError(t, model.DB.Model(&model.Log{}).Count(&logsBefore).Error)
	RecalculateTaskQuota(context.Background(), task, 4321, "provider reported usage")
	RecalculateTaskQuotaByTokens(context.Background(), task, 98765)
	...
	var logsAfter int64
	require.NoError(t, model.DB.Model(&model.Log{}).Count(&logsAfter).Error)
	assert.Equal(t, logsBefore, logsAfter)
	var persisted model.Task
	require.NoError(t, model.DB.First(&persisted, task.ID).Error)
	assert.Zero(t, persisted.Quota)
	assert.Equal(t, model.TaskBillingSourcePlatformExternal, persisted.PrivateData.BillingSource)
}

func TestProtectedOrdinaryTaskBillingFailsClosedAtEveryWriteSink(t *testing.T) {
	truncate(t)
	...
	seedUser(t, userID, 7000)
	seedToken(t, tokenID, userID, "sk-protected-native", 8000)
	task := makeTask(userID, 13, 500, tokenID, BillingSourceWallet, 0)
	require.NoError(t, model.DB.Create(task).Error)

	RecalculateTaskQuota(context.Background(), task, 900, "must fail closed")
	RecalculateTaskQuotaByTokens(context.Background(), task, 1234)
	require.False(t, RefundTaskQuota(context.Background(), task, "must fail closed"))
	recorder := httptest.NewRecorder()
	ginContext, _ := gin.CreateTestContext(recorder)
	LogTaskConsumption(ginContext, &relaycommon.RelayInfo{})

	var user model.User
	require.NoError(t, model.DB.First(&user, userID).Error)
	require.Equal(t, 7000, user.Quota)
	var token model.Token
	require.NoError(t, model.DB.First(&token, tokenID).Error)
	require.Equal(t, 8000, token.RemainQuota)
