...
func TestProtectedRelayHTTPEnvironmentAndDirectTransportRejectAmbientProxyAndTrust(t *testing.T) {
	for _, environment := range []string{
		...
		t.Run(environment, func(t *testing.T) {
			...
			require.EqualError(t, ValidateProtectedRelayHTTPEnvironment(), "protected Relay HTTP environment is invalid")
		})
	}
	...
	transport.DialContext = func(ctx context.Context, network, _ string) (net.Conn, error) {
		return (&net.Dialer{}).DialContext(ctx, network, targetServer.Listener.Addr().String())
	}
	client := newRelayHTTPClient(transport)
	request, err := http.NewRequest(http.MethodPost, "http://provider.example.invalid/v1/generations", nil)
	require.NoError(t, err)
	request.Header.Set("Authorization", "Bearer synthetic-provider-token")
	response, err := client.Do(request)
	require.NoError(t, err)
	require.NoError(t, response.Body.Close())
	require.Equal(t, int32(1), targetRequests.Load())
	require.Zero(t, proxyRequests.Load())
	require.False(t, proxyAuthorization.Load())
}
