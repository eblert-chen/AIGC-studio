...
func SetRelayRouter(router *gin.Engine) {
	...
	modelsRouter.Use(middleware.PlatformRelayOrTokenAuth())
	{
		modelsRouter.GET("", func(c *gin.Context) {
			if _, ok := middleware.GetPlatformRelayPrincipal(c); ok {
				controller.ListPlatformRelayModels(c)
				return
			}
			switch {
		...
		modelsRouter.GET("/:model", func(c *gin.Context) {
			if _, ok := middleware.GetPlatformRelayPrincipal(c); ok {
				controller.GetPlatformRelayModel(c)
				return
			}
			switch {
			case c.GetHeader("x-api-key") != "" && c.GetHeader("anthropic-version") != "":
	...
	geminiRouter.Use(middleware.RejectProtectedRelayNativeCompatibility())
	...
	geminiCompatibleRouter.Use(middleware.RejectProtectedRelayNativeCompatibility())
	...
	playgroundRouter.Use(middleware.RejectProtectedRelayNativeCompatibility())
	...
	relayV1Router.Use(middleware.RejectProtectedRelayNativeCompatibility())
	...
	relayMjRouter.Use(middleware.RejectProtectedRelayNativeCompatibility())
	relayMjRouter.Use(middleware.SystemPerformanceCheck())
	registerMjRouterGroup(relayMjRouter)

	relayMjModeRouter := router.Group("/:mode/mj")
	relayMjModeRouter.Use(middleware.RouteTag("relay"))
	relayMjModeRouter.Use(middleware.RejectProtectedRelayNativeCompatibility())
	...
	relaySunoRouter.Use(middleware.RejectProtectedRelayNativeCompatibility())
	...
	relayGeminiRouter.Use(middleware.RejectProtectedRelayNativeCompatibility())
