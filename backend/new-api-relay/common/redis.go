...
func InitRedisClient() (err error) {
	redisDSN := os.Getenv("REDIS_CONN_STRING")
	protectedDSN, protectedRootCAs, protected := protectedRelayRedisConfiguration()
	if protected {
		redisDSN = protectedDSN
	}
	if redisDSN == "" {
		RedisEnabled = false
	...
	if protected {
		if err := configureProtectedRelayRedisTLS(opt, protectedRootCAs); err != nil {
			FatalLog("failed to configure protected Relay Redis TLS")
		}
	}
...
func configureProtectedRelayRedisTLS(opt *redis.Options, rootCAs *x509.CertPool) error {
	if opt == nil || opt.TLSConfig == nil || rootCAs == nil || len(rootCAs.Subjects()) == 0 {
		return errors.New("protected Relay Redis TLS configuration is invalid")
	}
	host, _, err := net.SplitHostPort(opt.Addr)
	if err != nil || host == "" {
		return errors.New("protected Relay Redis TLS endpoint is invalid")
	}
	configuration := opt.TLSConfig.Clone()
	configuration.RootCAs = rootCAs.Clone()
