...
func TestFetchModelsRequestStopsWithSystemTaskContext(t *testing.T) {
	...
	go func() {
		_, err := getFetchModelsResponseBody(ctx, http.MethodGet, server.URL, &model.Channel{}, nil)
		result <- err
	}()
	select {
	case <-started:
	case <-time.After(time.Second):
		t.Fatal("upstream model request did not start")
	}
	cancel()
	select {
	case err := <-result:
	...
	case <-time.After(time.Second):
		t.Fatal("upstream model request ignored system task cancellation")
	}
	select {
...
func TestCanceledSystemModelUpdateCannotBeStampedSucceeded(t *testing.T) {
	db := setupModelListControllerTestDB(t)
	require.NoError(t, db.AutoMigrate(&model.SystemTask{}, &model.SystemTaskLock{}))
	task, err := model.CreateSystemTask(model.SystemTaskTypeModelUpdate, nil, nil)
	require.NoError(t, err)
	claimed, ok, err := model.ClaimSystemTask(task.ID, task.Type, "cancel-runner", common.GetTimestamp()+60)
	require.NoError(t, err)
	require.True(t, ok)

	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	modelUpdateHandler{}.Run(ctx, claimed, "cancel-runner")

	reloaded, err := model.GetSystemTaskByTaskID(task.TaskID)
	require.NoError(t, err)
	require.Equal(t, model.SystemTaskStatusFailed, reloaded.Status)
	require.Contains(t, reloaded.Error, context.Canceled.Error())
}
