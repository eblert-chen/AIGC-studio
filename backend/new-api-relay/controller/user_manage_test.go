...
func TestManageUserCannotMutateProtectedPlatformRelayServicePrincipal(t *testing.T) {
	db := setupManageUserTestDB(t)
	t.Setenv("RELAY_DATABASE_ROLE_ATTESTATION_REQUIRED", "true")
	principal := model.User{
		Username: "RSVC_managed", Password: "!invalid-service-password!", Role: common.RoleCommonUser,
		Status: common.UserStatusEnabled, Group: "default", AuthVersion: 1, AffCode: "service-aff",
	}
	require.NoError(t, db.Create(&principal).Error)

	requests := []string{
		fmt.Sprintf(`{"id":%d,"action":"disable"}`, principal.Id),
		fmt.Sprintf(`{"id":%d,"action":"delete"}`, principal.Id),
		fmt.Sprintf(`{"id":%d,"action":"add_quota","mode":"add","value":10}`, principal.Id),
		fmt.Sprintf(`{"id":%d,"action":"add_quota","mode":"subtract","value":10}`, principal.Id),
		fmt.Sprintf(`{"id":%d,"action":"add_quota","mode":"override","value":10}`, principal.Id),
	}
	for _, request := range requests {
		recorder := performManageUserRequest(t, request)
		require.NotContains(t, recorder.Body.String(), `"success":true`)
	}

	var persisted model.User
	require.NoError(t, db.First(&persisted, principal.Id).Error)
	require.Equal(t, common.UserStatusEnabled, persisted.Status)
	require.Zero(t, persisted.Quota)
	require.EqualValues(t, 1, persisted.AuthVersion)
}
