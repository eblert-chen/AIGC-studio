...
func getVertexTaskKey(channel *model.Channel, task *model.Task) (string, error) {
	if channel == nil || task == nil || task.ChannelId != channel.Id {
		return "", fmt.Errorf("vertex task channel identity is invalid")
	}
	key, err := model.ResolveTaskProviderCredential(task)
	if err != nil {
		return "", fmt.Errorf("vertex provider credential version is unavailable: %w", err)
	}
	return key, nil
}
