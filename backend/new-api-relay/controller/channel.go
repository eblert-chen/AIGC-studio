	...
	if enableTagMode {
		...
		for _, tag := range tags {
			...
			err := sortOptions.Apply(buildChannelListQuery(groupFilter, statusFilter, typeFilter).Where("tag = ?", *tag)).
				Omit("key", "credential_set_version").
...
type addChannelCredentialInput struct {
	Channel *struct {
		Key *string `json:"key"`
	} `json:"channel"`
}
...
func AddChannel(c *gin.Context) {
	...
	credentialInput := addChannelCredentialInput{}
	if err := common.Unmarshal(rawBody, &credentialInput); err != nil {
		common.ApiError(c, err)
		return
	}
	...
	for _, key := range keys {
		...
		if addChannelRequest.BatchAddSetKeyPrefix2Name && len(keys) > 1 {
			// Never persist a raw credential prefix in a channel name. A short
			// controlled fingerprint is sufficient for operator distinction.
			localChannel.Name = fmt.Sprintf("%s key-%s", localChannel.Name, model.ProviderChannelCredentialFingerprintPrefix(localChannel.Key, 8))
		}
...
func UpdateChannel(c *gin.Context) {
	...
	if rawKey, ok := requestData["key"]; ok {
...
type KeyStatus struct {
	...
	KeyPreview   string `json:"key_preview"` // one-way fingerprint, never raw key material
}
...
func ManageMultiKeys(c *gin.Context) {
	...
	case "get_key_status":
		...
		for i, key := range keys {
			...
			keyPreview := "sha256:" + model.ProviderChannelCredentialFingerprintPrefix(key, 12)
