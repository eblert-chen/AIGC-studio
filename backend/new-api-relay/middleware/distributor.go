...
// SetupContextForPinnedChannel configures one exact channel key without
// advancing new-api's random/polling key selector. Admission has already been
// decided durably by the generation route table before this function runs.
func SetupContextForPinnedChannel(c *gin.Context, channel *model.Channel, modelName string, key string, keyIndex int) *types.NewAPIError {
	c.Set("original_model", modelName)
	if channel == nil {
		return types.NewError(errors.New("channel is nil"), types.ErrorCodeGetChannelFailed, types.ErrOptionWithSkipRetry())
	}
	if strings.TrimSpace(key) == "" || keyIndex < 0 {
		return types.NewError(errors.New("pinned channel key is invalid"), types.ErrorCodeChannelNoAvailableKey, types.ErrOptionWithSkipRetry())
	}
	common.SetContextKey(c, constant.ContextKeyChannelId, channel.Id)
	common.SetContextKey(c, constant.ContextKeyChannelName, channel.Name)
	common.SetContextKey(c, constant.ContextKeyChannelType, channel.Type)
