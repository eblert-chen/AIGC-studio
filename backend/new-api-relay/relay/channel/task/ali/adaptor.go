...
func isWan27Model(model string) bool {
	return isWan27T2VModel(model) || isWan27I2VModel(model)
}
...
// wan27OutputFromSize converts the native bridge's pixel dimensions to the
// current Wan2.7 resolution + ratio protocol. Wan2.7 does not accept the old
// parameters.size field.
func wan27OutputFromSize(size string) (resolution string, ratio string, err error) {
	...
	case "1440x1440":
		return "1080P", "1:1", nil
	case "1648x1248", "1632x1248":
		return "1080P", "4:3", nil
	case "1248x1648", "1248x1632":
		return "1080P", "3:4", nil
	default:
		return "", "", fmt.Errorf("unsupported Wan2.7 output size %q", size)
	}
}

func validateWan27Output(aliReq *AliVideoRequest) error {
	if aliReq.Parameters == nil {
		return errors.New("Wan2.7 parameters are required")
	}
	if aliReq.Parameters.Size != "" {
		resolution, ratio, err := wan27OutputFromSize(aliReq.Parameters.Size)
		if err != nil {
			return err
		}
		aliReq.Parameters.Resolution = resolution
		if isWan27T2VModel(aliReq.Model) {
	...
	if aliReq.Parameters.Resolution != "720P" && aliReq.Parameters.Resolution != "1080P" {
		return fmt.Errorf("Wan2.7 resolution must be 720P or 1080P")
	}
	if aliReq.Parameters.Duration < 2 || aliReq.Parameters.Duration > 15 {
	...
	if utf8.RuneCountInString(aliReq.Input.Prompt) > 5000 {
		return fmt.Errorf("Wan2.7 prompt exceeds 5000 characters")
	}
	...
	if isWan27T2VModel(aliReq.Model) {
		...
		default:
			return fmt.Errorf("Wan2.7 text-to-video ratio is not supported: %s", aliReq.Parameters.Ratio)
		}
		if aliReq.Input.ImgURL != "" || aliReq.Input.FirstFrameURL != "" || aliReq.Input.LastFrameURL != "" || len(aliReq.Input.Media) > 0 {
			return fmt.Errorf("Wan2.7 text-to-video does not accept image or video media")
		}
		return nil
	}

	if aliReq.Parameters.Ratio != "" {
		return fmt.Errorf("Wan2.7 image-to-video follows the input aspect ratio and does not accept parameters.ratio")
	}
	return nil
}
...
func validateWan27Media(media []AliVideoMedia) error {
	seen := make(map[string]struct{}, len(media))
	for index := range media {
		media[index].Type = strings.TrimSpace(media[index].Type)
		media[index].URL = strings.TrimSpace(media[index].URL)
		if media[index].URL == "" {
			return fmt.Errorf("Wan2.7 input.media[%d].url is required", index)
		}
		switch media[index].Type {
		case "first_frame", "last_frame", "driving_audio", "first_clip":
		default:
			return fmt.Errorf("Wan2.7 input.media[%d].type is not supported", index)
		}
		if _, duplicate := seen[media[index].Type]; duplicate {
			return fmt.Errorf("Wan2.7 input.media type %q may appear only once", media[index].Type)
		}
		seen[media[index].Type] = struct{}{}
	}

	_, firstFrame := seen["first_frame"]
	_, lastFrame := seen["last_frame"]
	_, drivingAudio := seen["driving_audio"]
	_, firstClip := seen["first_clip"]
	valid := (firstFrame && !firstClip) || (firstClip && !firstFrame && !drivingAudio)
	if !valid || (lastFrame && !firstFrame && !firstClip) {
		return fmt.Errorf("Wan2.7 input.media contains an unsupported material combination")
	}
	return nil
}

func platformGenerationMode(req relaycommon.TaskSubmitReq) string {
	if req.Metadata == nil {
		return ""
	}
...
func normalizeWan27I2VInput(aliReq *AliVideoRequest, req relaycommon.TaskSubmitReq) error {
	...
	if len(aliReq.Input.Media) == 0 {
		...
		firstFrameURL := firstNonEmpty(aliReq.Input.FirstFrameURL, aliReq.Input.ImgURL)
		lastFrameURL := strings.TrimSpace(aliReq.Input.LastFrameURL)
		firstClipURL := ""
		if platformGenerationMode(req) == "video_to_video" {
			firstClipURL = strings.TrimSpace(req.InputReference)
			// In video continuation mode, the bridge's Image mirror is the
			// optional last frame, never a first_frame input.
			firstFrameURL = ""
		} else if firstFrameURL == "" {
			firstFrameURL = strings.TrimSpace(req.InputReference)
		}
		if firstFrameURL != "" && len(images) > 0 {
			// The public bridge sends Image plus the full Images slice. Treat the
			// first list entry as the mirrored first frame, even if a direct API
			// caller supplied a different-but-equivalent URL representation.
			if strings.TrimSpace(req.Image) != "" || images[0] == firstFrameURL {
				images = images[1:]
			}
		}
		if firstClipURL == "" && firstFrameURL == "" && len(images) > 0 {
			firstFrameURL = images[0]
			images = images[1:]
		}
		if lastFrameURL == "" && len(images) > 0 {
			lastFrameURL = images[0]
		}
	...
	if err := validateWan27Media(aliReq.Input.Media); err != nil {
		return err
	}
...
func (a *TaskAdaptor) convertToAliRequest(info *relaycommon.RelayInfo, req relaycommon.TaskSubmitReq) (*AliVideoRequest, error) {
	...
	// Wan2.7 uses resolution + ratio. Older Wan releases keep their legacy
	// size/resolution mapping below.
	if isWan27Model(upstreamModel) {
		if strings.TrimSpace(req.Size) != "" {
			resolution, ratio, err := wan27OutputFromSize(req.Size)
			if err != nil {
				return nil, err
			}
			aliReq.Parameters.Resolution = resolution
			if isWan27T2VModel(upstreamModel) {
				aliReq.Parameters.Ratio = ratio
			}
		} else {
			aliReq.Parameters.Resolution = "720P"
	...
	// 从 metadata 中提取额外参数
	if req.Metadata != nil && !isPinnedPlatformRoute(info) {
	...
	if isWan27Model(aliReq.Model) {
		if err := validateWan27Output(aliReq); err != nil {
			return nil, err
		}
	}
