...
func GenRelayInfo(c *gin.Context, relayFormat types.RelayFormat, request dto.Request, ws *websocket.Conn) (*RelayInfo, error) {
	...
	case types.RelayFormatTask:
		...
		info.TaskRelayInfo = &TaskRelayInfo{
			PublicTaskID:        common.GetContextKeyString(c, constant.ContextKeyPlatformGenerationPublicTaskID),
			PinnedProviderRoute: common.GetContextKeyBool(c, constant.ContextKeyPlatformGenerationPinnedRoute),
		}
...
type TaskRelayInfo struct {
	Action              string
	OriginTaskID        string
	PinnedProviderRoute bool
	// ProviderCredentialVersion is an opaque encrypted-vault reference. It is
	// carried only in process between pre-POST staging and final Task creation;
	// provider key material must never be placed here.
	ProviderCredentialTenantID string
