...
type Channel struct {
	...
	// Key is provider credential material hydrated only at a trusted provider-call
	// or explicit root key-view boundary. It is never mapped to the legacy
	// channels.key column and can never be serialized by an ordinary Channel
	// response.
	Key string `json:"-" gorm:"-"`
	// LegacyKey keeps the upstream database column available during the atomic
	// migration. The migration encrypts and clears it before installing database
	// guards that permanently reject every subsequent non-empty write.
	LegacyKey string `json:"-" gorm:"column:key;not null;default:''"`
	// CredentialSetVersion is an opaque internal pointer to an immutable,
	// authenticated whole-key-set snapshot. API responses must never expose it.
	CredentialSetVersion string  `json:"-" gorm:"column:credential_set_version;type:varchar(36);index"`
	...
	// ControlRevision is an internal optimistic-concurrency counter used only by
	// the Platform channel-control facade. It must never be serialized through
	// new-api's native channel management responses.
	ControlRevision int64 `json:"-" gorm:"not null;default:0"`
...
// GetKeyAt returns the exact configured account key at a stable index without
// advancing the channel's random or polling selector. It is used only after a
// durable generation route admission has already selected that account.
func (channel *Channel) GetKeyAt(index int) (string, error) {
	keys := channel.GetKeys()
	if index < 0 || index >= len(keys) {
		return "", fmt.Errorf("channel key index %d is out of range", index)
	}
	if !channel.ChannelInfo.IsMultiKey && index != 0 {
		return "", fmt.Errorf("single-key channel cannot use key index %d", index)
	}
	if strings.TrimSpace(keys[index]) == "" {
		return "", fmt.Errorf("channel key index %d is empty", index)
	}
	...
	case constant.MultiKeyModePolling:
		...
		for i := 0; i < len(keys); i++ {
			...
			if getStatus(idx) == common.ChannelStatusEnabled {
				...
				CacheUpdateChannelPollingIndex(channel.Id, next)
...
func (channel *Channel) Save() error {
	// control_revision is database-owned. Omitting it prevents a stale native
	// Channel value from moving the monotonic revision backwards while the
	// database trigger derives the next revision from the locked OLD row.
	return DB.Omit("control_revision").Save(channel).Error
}
...
func GetAllChannels(startIdx int, num int, selectAll bool, idSort bool, sortOptions ...ChannelSortOptions) ([]*Channel, error) {
	...
	if selectAll {
		...
		err = order.Apply(DB).Limit(num).Offset(startIdx).Omit("key", "credential_set_version").Find(&channels).Error
	}
	return channels, err
}
...
func GetChannelsByTag(tag string, idSort bool, selectAll bool, sortOptions ...ChannelSortOptions) ([]*Channel, error) {
	...
	if !selectAll {
		query = query.Omit("key", "credential_set_version")
	}
...
func SearchChannels(keyword string, group string, model string, idSort bool, sortOptions ...ChannelSortOptions) ([]*Channel, error) {
	...
	// 构造基础查询
	baseQuery := DB.Model(&Channel{}).Omit("key", "credential_set_version")

	// 构造WHERE子句
	// Provider credentials are intentionally not searchable. Even equality
	// searches disclose whether guessed credential material exists in the
	// database and force unsafe plaintext handling.
...
func GetChannelById(id int, selectAll bool) (*Channel, error) {
	...
	if selectAll {
		...
		err = DB.Omit("key", "credential_set_version").First(channel, "id = ?", id).Error
	}
	if err != nil {
		return nil, err
	}
...
func (channel *Channel) Update() error {
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		// BeforeUpdate resolves an explicit non-empty runtime Key into a new
		// immutable credential-set version (or preserves the current version for
		// an empty Key). Keep credential_set_version in this UPDATE so creating
		// the version row and switching the channel reference commit atomically.
		// The legacy plaintext key column remains omitted permanently.
		if err := tx.Model(channel).Omit("key", "control_revision").Updates(channel).Error; err != nil {
			return err
		}
...
func (channel *Channel) Delete() error {
	return DB.Transaction(func(tx *gorm.DB) error {
		var existing Channel
		if err := lockForUpdate(tx.Where("id = ?", channel.Id)).First(&existing).Error; errors.Is(err, gorm.ErrRecordNotFound) {
			// Channel deletion has historically been idempotent. Keep that API
			// contract (and its proxy-cache reset) while still rejecting a
			// corrupt dangling active generation admission for this identity.
			return rejectPlatformGenerationChannelMutationTx(tx, channel.Id)
		} else if err != nil {
			return err
		}
		if err := rejectPlatformGenerationChannelMutationTx(tx, channel.Id); err != nil {
...
var ErrPlatformGenerationChannelInUse = errors.New("channel has active Platform generation tasks")

// rejectPlatformGenerationChannelMutationTx protects long-lived sticky tasks.
// The Channel row is already locked by callers, so a concurrent new admission
// either committed first and is visible here, or observes the mutation and
// fails its transactional channel/key check before a provider POST.
func rejectPlatformGenerationChannelMutationTx(tx *gorm.DB, channelID int) error {
	...
	var active int64
	err := tx.Table("platform_generation_route_admissions AS admissions").
		Joins("JOIN platform_generation_provider_routes AS routes ON routes.id = admissions.route_id").
		Where("routes.channel_id = ? AND admissions.slot_held = ?", channelID, true).
		Count(&active).Error
	if err != nil {
		return err
	}
...
func platformGenerationChannelTransportChanged(existing Channel, update Channel) bool {
	if update.Key != "" && update.Key != existing.Key {
		return true
	}
	if update.Type != 0 && update.Type != existing.Type {
		return true
	}
	if update.BaseURL != nil && !channelStringPointersEqual(update.BaseURL, existing.BaseURL) {
		return true
	}
	if update.OpenAIOrganization != nil && !channelStringPointersEqual(update.OpenAIOrganization, existing.OpenAIOrganization) {
		return true
	}
	if update.Setting != nil && !channelStringPointersEqual(update.Setting, existing.Setting) {
		return true
	}
	if update.Other != "" && update.Other != existing.Other {
		return true
	}
	if update.OtherSettings != "" && update.OtherSettings != existing.OtherSettings {
		return true
	}
	if update.ParamOverride != nil && !channelStringPointersEqual(update.ParamOverride, existing.ParamOverride) {
		return true
	}
	if update.HeaderOverride != nil && !channelStringPointersEqual(update.HeaderOverride, existing.HeaderOverride) {
		return true
	}
	return false
}

func channelStringPointersEqual(left *string, right *string) bool {
...
func GetChannelsByIds(ids []int) ([]*Channel, error) {
	var channels []*Channel
	err := DB.Where("id in (?)", ids).Omit("key", "credential_set_version").Find(&channels).Error
	return channels, err
}
...
func BatchSetChannelTag(ids []int, tag *string) error {
	...
	// Read through the same transaction that owns the tag update. Using the
...
func GetChannelsByType(startIdx int, num int, idSort bool, channelType int) ([]*Channel, error) {
	...
	err := DB.Where("type = ?", channelType).Order(order).Limit(num).Offset(startIdx).Omit("key", "credential_set_version").Find(&channels).Error
	return channels, err
}
