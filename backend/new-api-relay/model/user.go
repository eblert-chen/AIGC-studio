...
func UpdateUserSetting(userId int, setting dto.UserSetting) error {
	...
	if err = withProtectedPlatformRelayServiceUserMutation(DB, userId, func(tx *gorm.DB) error {
...
func inviteUser(inviterId int) (err error) {
	...
	return DB.Transaction(func(tx *gorm.DB) error {
		if err := rejectProtectedPlatformRelayServiceUserMutation(tx, user.Id); err != nil {
			return err
		}
...
func (user *User) prepareForInsert(tx *gorm.DB) error {
	if protectedPlatformRelayUserIdentityIsReserved(user.Username, user.Remark) {
...
func (user *User) UpdateGitHubId(newGitHubId string) error {
	...
	return withProtectedPlatformRelayServiceUserMutation(DB, user.Id, func(tx *gorm.DB) error {
...
func ResetUserPasswordByEmail(email string, password string) error {
	...
	if err = DB.Transaction(func(tx *gorm.DB) error {
		var users []User
		if err := lockForUpdate(tx).Unscoped().Where("LOWER(email) = ?", email).Limit(2).Find(&users).Error; err != nil {
			return err
		}
		switch len(users) {
		case 0:
...
func increaseUserQuota(id int, quota int) (err error) {
	return withProtectedPlatformRelayServiceUserMutation(DB, id, func(tx *gorm.DB) error {
...
func UpdateUserLastLoginAt(id int) {
	if err := withProtectedPlatformRelayServiceUserMutation(DB, id, func(tx *gorm.DB) error {
		return tx.Model(&User{}).Where("id = ?", id).Update("last_login_at", common.GetTimestamp()).Error
	}); err != nil {
...
func UpdateUserUsedQuotaAndRequestCount(id int, quota int) {
	if common.BatchUpdateEnabled && !RelayDatabaseRoleAttestationRequired() {
...
func updateUserUsedQuotaAndRequestCount(id int, quota int, count int) {
	err := withProtectedPlatformRelayServiceUserMutation(DB, id, func(tx *gorm.DB) error {
		return tx.Model(&User{}).Where("id = ?", id).Updates(
