...
func GetTokenByKey(key string, fromDB bool) (token *Token, err error) {
	// Protected Relay credentials can be rotated while Redis survives the API
	// processes. Never authenticate from a token cache in an attested staging
	// or production deployment: an old service-principal key may otherwise stay
	// usable until its cache TTL after the database transaction commits. This is
	// the lowest shared boundary for TokenAuth, TokenAuthReadOnly and quota
	// lookups, so every protected token decision observes current database state.
	protectedDirectRead := RelayDatabaseRoleAttestationRequired()
	if protectedDirectRead {
		fromDB = true
	}
	defer func() {
		// Update Redis cache asynchronously on successful DB read
		if !protectedDirectRead && shouldUpdateRedis(fromDB, err) && token != nil {
			gopool.Go(func() {
				if err := cacheSetToken(*token); err != nil {
					common.SysLog("failed to update user status cache: " + err.Error())
				}
	...
	fromDB = true
	err = DB.Where(clause.Eq{Column: clause.Column{Name: "key"}, Value: key}).First(&token).Error
	return token, err
}

func (token *Token) Insert() error {
	if protectedPlatformRelayTokenNameIsReserved(token.Name) {
		return ErrPlatformRelayServicePrincipalNamespaceReserved
	}
	if RelayDatabaseRoleAttestationRequired() {
		// Rotation holds SHARE ROW EXCLUSIVE on the token table while changing
		// keys. An ordinary insert that was queued behind that lock must converge
		// to a secret-free conflict after commit, never a PostgreSQL unique-error
		// DETAIL containing the rotated credential.
		insert := DB.Clauses(clause.OnConflict{DoNothing: true}).Create(token)
		if insert.Error != nil || insert.RowsAffected != 1 {
			return ErrTokenKeyConflict
		}
		return nil
	}
	var err error
...
func (token *Token) Update() (err error) {
	err = withProtectedPlatformRelayTokenMutation(DB, token.Id, token.Name, func(tx *gorm.DB) error {
...
func increaseTokenQuota(id int, quota int) (err error) {
	return withProtectedPlatformRelayTokenMutation(DB, id, "", func(tx *gorm.DB) error {
		return tx.Model(&Token{}).Where("id = ?", id).Updates(
...
func BatchDeleteTokens(ids []int, userId int) (int, error) {
	...
	for _, token := range tokens {
		if protectedPlatformRelayTokenNameIsReserved(token.Name) {
			tx.Rollback()
			return 0, ErrPlatformRelayServicePrincipalNamespaceReserved
		}
	}
