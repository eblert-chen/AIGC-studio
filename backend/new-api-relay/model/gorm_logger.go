...
var (
	gormLogWriterMu sync.RWMutex
	gormLogWriter   io.Writer = os.Stdout
)

// SetGormLogWriterForOfflineCommand keeps an offline command's stdout reserved
// for its single machine-readable receipt. It must be called before InitDB.
func SetGormLogWriterForOfflineCommand(writer io.Writer) {
	if writer == nil {
		return
	}
	gormLogWriterMu.Lock()
	gormLogWriter = writer
	gormLogWriterMu.Unlock()
}

func currentGormLogWriter() io.Writer {
	gormLogWriterMu.RLock()
	defer gormLogWriterMu.RUnlock()
...
func newGormConfig(prepareStmt bool) *gorm.Config {
	return &gorm.Config{
		...
		Logger:      newGormLogger(currentGormLogWriter()),
	}
}
