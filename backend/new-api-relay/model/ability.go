...
func filterAbilitiesByRequestPathAndModel(abilities []Ability, requestPath string, model string) []Ability {
	...
	if err := DB.Where("id IN ?", channelIds).Omit("key", "credential_set_version").Find(&channels).Error; err != nil {
...
func FixAbility() (int, int, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		// DELETE is ordinary DML and works under the exact runtime manifest;
		// TRUNCATE is intentionally forbidden because it bypasses that boundary.
		if err := tx.Exec("DELETE FROM abilities").Error; err != nil {
			return fmt.Errorf("delete abilities: %w", err)
		}
		var channels []*Channel
		if err := tx.Model(&Channel{}).Omit("key", "credential_set_version").Find(&channels).Error; err != nil {
			return err
		}
