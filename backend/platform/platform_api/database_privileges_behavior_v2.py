"""Frozen PostgreSQL 16 attestation behavior for Platform Alembic 0037.

This module and :mod:`database_privileges_v2` are one historical security
...
revision 0037 must continue importing this module directly. PostgreSQL system
semantics remain pinned to the independently frozen v1 server baseline.
"""
...
def protected_platform_runtime_requested_v2() -> bool:
...
def _protected_database_release_environment_v2() -> str:
...
def expected_platform_table_acl(
    ...
    return policy_v2.EXPECTED_TABLE_ACL
...
def _validate_principals(
    ...
    *,
    ...
    expected_names = set(policy_v2.DATABASE_ROLE_BY_PROCESS.values())
    ...
    for role_name, principal in by_name.items():
        ...
        if (
            ...
            != policy_v2.DATABASE_ROLE_COMMENT_BY_PROCESS[process_role]
...
def validate_platform_database_evidence(
    ...
    *,
    ...
    _require_frozen_policy(policy)
    if process_role not in policy_v2.DATABASE_ROLE_BY_PROCESS:
        _fail("process role")
    expected_user = policy_v2.DATABASE_ROLE_BY_PROCESS[process_role]
    ...
    if evidence.database_owner != policy_v2.MIGRATION_DATABASE_ROLE:
    ...
    if not platform_postgres16_release_evidence_is_qualified(
        environment=_protected_database_release_environment_v2(),
    ...
    if not require_runtime_acl:
        if evidence.alembic_heads == (policy_v2.ALEMBIC_HEAD,):
            ...
            return
        if evidence.alembic_heads == ("0036_platform_db_roles",):
            if evidence.default_acl != policy_v2.EXPECTED_DEFAULT_ACL:
                _fail("pre-migration default privileges")
            return
        if evidence.alembic_heads:
            _fail("pre-migration head")
...
def validate_platform_database_acl_evidence(
    ...
    *,
    ...
    if (
        normalized_database_acl != policy_v2.EXPECTED_DATABASE_ACL
        or any(
            grantor != policy_v2.MIGRATION_DATABASE_ROLE or is_grantable
    ...
    if (
        normalized_schema_acl != policy_v2.EXPECTED_SCHEMA_ACL
        or any(
            grantor
            not in {policy_v2.MIGRATION_DATABASE_ROLE, "pg_database_owner"}
            or is_grantable
            for _, _, grantor, is_grantable in evidence.schema_acl
        )
    ):
        _fail("schema privileges")
    if evidence.table_names != policy_v2.TABLES | {"alembic_version"}:
    ...
    if (
        normalized_table_acl != policy_v2.EXPECTED_TABLE_ACL
        or any(
            grantor != policy_v2.MIGRATION_DATABASE_ROLE or is_grantable
    ...
    if evidence.catalog_sha256 != policy_v2.CATALOG_SHA256:
        _fail("catalog fingerprint")
    if require_head and evidence.alembic_heads != (policy_v2.ALEMBIC_HEAD,):
        _fail("migration head")


def _role_predicate(prefix: str) -> tuple[str, dict[str, str]]:
    values = tuple(policy_v2.DATABASE_ROLE_BY_PROCESS.values())
...
_CATALOG_PROJECTIONS = (
    ...
    (
        ...
        # the internal names here would make equal fresh databases hash
        # differently.  User-created triggers remain fully projected.
...
def validate_platform_migration_source_state(
    ...
    *,
    ...
    if "alembic_version" not in table_names:
        if table_names or catalog_sha256 != policy_v2.EMPTY_CATALOG_SHA256:
    ...
    expected = policy_v2.MIGRATION_SOURCE_CATALOG_SHA256_BY_HEAD.get(heads[0])
...
def collect_platform_database_evidence(
    ...
    *,
    ...
    system_acl_count = int(
        system_acl_sha256
        != policy_v2.POSTGRES16_SYSTEM_ACL_BY_SYSTEM_SEMANTIC_SHA256.get(
            system_semantic_sha256
        )
    )
    release_environment = _protected_database_release_environment_v2()
    require_pgaudit = (
    ...
    foreign_owned_object_count = int(
        connection.scalar(
            ...
            {"migration_role": policy_v2.MIGRATION_DATABASE_ROLE, **role_parameters},
        )
        or 0
    )
    column_acl_count = int(
    ...
    legacy_pending_work_observable = legacy_tables_exist and bool(
        connection.scalar(
            text(
                "SELECT has_table_privilege(current_user,"
                "'public.generation_tasks','SELECT') AND "
                "has_table_privilege(current_user,"
                "'public.relay_submission_outbox','SELECT')"
            )
        )
    )
    if legacy_pending_work_observable:
        legacy_pending_work_count = int(
            connection.scalar(
                text(
                    "SELECT "
...
def attest_platform_database_connection(
    ...
    *,
    ...
    _require_frozen_policy(policy)
    if not protected_platform_runtime_requested_v2():
        return
...
def install_platform_database_connection_attestation(
    ...
    _require_frozen_policy(policy_v2)
    if not protected_platform_runtime_requested_v2():
        return
    if process_role not in policy_v2.DATABASE_ROLE_BY_PROCESS:
        _fail("process role")
...
def assert_platform_database_manifest_matches_metadata(
    ...
    if frozenset(table_names) != policy_v2.TABLES:
        raise AssertionError("Platform database privilege manifest is stale")


def validate_privilege_manifest() -> None:
    expected_roles = set(policy_v2.DATABASE_ROLE_BY_PROCESS) - {"migration"}
    if set(policy_v2.PRIVILEGES_BY_PROCESS) != expected_roles:
        raise AssertionError("Platform database process manifest is incomplete")
    allowed = {"SELECT", "INSERT", "UPDATE", "DELETE"}
    for privileges_by_table in policy_v2.PRIVILEGES_BY_PROCESS.values():
        if not set(privileges_by_table).issubset(policy_v2.TABLES):
